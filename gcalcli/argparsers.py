...
PROGRAM_OPTIONS = {
    "--client-id": {
        "default": gcalcli.__API_CLIENT_ID__,
        "type": str,
        "help": "API client_id",
    },
    "--client-secret": {
        "default": gcalcli.__API_CLIENT_SECRET__,
        "type": str,
...
def get_argument_parser():
    ...
    sub.add_parser(
        "search",
        parents=[details_parser, output_parser, search_parser],
    ...
    calw = sub.add_parser(
        "calw",
        parents=[details_parser, output_parser, cal_query_parser],
    ...
    add.add_argument("--who", default=[], type=str, action="append", help="Event title")
    ...
    _import.add_argument("file", type=argparse.FileType("r"), nargs="?", default=None)
    _import.add_argument(
        "--verbose", "-v", action="count", help="Be verbose on imports"
    )
    _import.add_argument(
        "--dump", "-d", action="store_true", help="Print events and don't import"
    )
